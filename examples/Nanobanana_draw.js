...
declare const OkHttp: any;
declare const Tools: {
    Files: {
        mkdir: (path: string) => Promise<{ successful: boolean; details?: string }>;
        download: (url: string, filePath: string) => Promise<{ successful: boolean; details?: string }>;
    };
};
...
const nanobananaDraw = (function () {
    ...
    // API配置
    const API_ENDPOINT: string = "https://grsai.dakka.com.cn/v1/draw/nano-banana";
    const RESULT_ENDPOINT: string = "https://grsai.dakka.com.cn/v1/draw/result";
    const DEFAULT_MODEL: string = "nano-banana-pro";
    // Android 实际路径为 /sdcard/Download，对应系统中文名"下载"
    const DOWNLOAD_ROOT: string = "/sdcard/Download";
    const OPERIT_DIR: string = `${DOWNLOAD_ROOT}/Operit`;
    const DRAWS_DIR: string = `${OPERIT_DIR}/draws`;
    
    // 轮询配置
    const POLL_INTERVAL: number = 5000;      // 每5秒查询一次
    const MAX_WAIT_TIME: number = 300000;    // 最多等待5分钟

    function getApiKey(): string {
        const apiKey: string | undefined = getEnv("NANOBANANA_API_KEY");
        if (!apiKey) {
    ...
    function buildFileName(prompt: string, customName?: string): string {
        if (customName && customName.trim().length > 0) {
            return sanitizeFileName(customName);
        }
        const shortPrompt: string = prompt.length > 40 ? `${prompt.substring(0, 40)}...` : prompt;
        const base: string = sanitizeFileName(shortPrompt || "image");
        const timestamp: number = Date.now();
        return `${base}_${timestamp}`;
    }

    async function ensureDirectories(): Promise<void> {
        const dirs: string[] = [DOWNLOAD_ROOT, OPERIT_DIR, DRAWS_DIR];
    ...
    async function callNanobananaApi(params: {
        ...
        const model: string = (params.model && params.model.trim().length > 0) 
            ? params.model.trim() 
            : DEFAULT_MODEL;
    ...
    async function pollForResult(taskId: string): Promise<string> {
        ...
        // 构建结果查询请求
        const requestBody: string = JSON.stringify({ id: taskId });
        const headers: Record<string, string> = {
            "accept": "application/json",
            "content-type": "application/json",
            "Authorization": `Bearer ${apiKey}`
    ...
    async function draw_image(params: DrawImageParams): Promise<DrawImageResult> {
        ...
        // 添加辅助函数来解析URL数组
        function parseImageUrls(image_urls: string | string[] | undefined): string[] {
            // 如果已经是数组，直接过滤空值返回
        ...
        // 步骤2: 轮询等待任务完成
        const imageUrl: string = await pollForResult(taskId);

        const ext: string = guessExtensionFromUrl(imageUrl);
        const baseName: string = buildFileName(prompt, params.file_name);
        const filePath: string = `${DRAWS_DIR}/${baseName}.${ext}`;
        ...
        const markdown: string = `![AI生成的图片](${fileUri})`;
    ...
    async function draw_image_wrapper(params: DrawImageParams): Promise<void> {
        try {
